...
let nameFieldDisplay = document.getElementById("nameFieldDisplay");
...
let allValidNameFieldList = getValidNameFieldMetaList(allInfo);

//根据是否有附件字段进行判断:
// another in optUpdate
if (allAttachmentFieldList === false) { //当前多维表格无附件字段
  ...
  nameType.addEventListener("change", displayUpdate);
  displayUpdate();
...
function getValidNameFieldMetaList(info) {
  let res = [];
  for (let i = 0; i < info.length; i++) {
    //从info中获取附件字段信息
    let iAttachmentFieldMetaList = info[i].fieldMetaList.filter(item => item.type !== 7);
    //输出附件字段的MetaList信息。
    let attachmentFieldInfo = {
      tableId: info[i].tableId,
      tableName: info[i].tableName,
...
function optUpdate() {
  ...
  if (selectedTableName != "0") { //选中了某一个数据表
    ...
    //update name field
    optionUpdateByTableSeleceted(xT.value, allValidNameFieldList, "fieldMetaList", nameField);
    nameField.value = nameField.options['0'].value; // Or will block the download
  }
}

function displayUpdate() {
  if (Number(nameType.value) === 2) {
    nameFieldDisplay.style.display = "";
  } else {
...
async function mainDownload() {
  ...
  let oNameFieldId = nameField.value;
  ...
  if (selectedNameType === 0 && downloadMethodType === 0) {//使用原文件名,文件逐个下载
    ...
    mode = "2";
  } else if (selectedNameType !== 0 && downloadMethodType === 0) {//使用索引作为文件名,文件逐个下载
    mode = "3";
  } else if (selectedNameType !== 0 && downloadMethodType === 1) {//使用索引作为文件名，zip打包下载
    mode = "4";
  }
  ...
  if (mode === "1" || mode === "3") {
    for (let i = 0; i < myCellsList.length; i++) {
      let fileInfo = await getFileInfo(oTable, myCellsList[i].fieldId, myCellsList[i].recordId, selectedNameType, indexFieldId, oNameFieldId);
...
async function getFileInfo(oTable, fieldId, recordId, nameType, indexFieldId, nameFieldId) {
  let fileList = [];
  let oCell = await oTable.getCellValue(fieldId, recordId);
  var oCellIndexInfo;
  if (nameType === 1) {
    oCellIndexInfo = await oTable.getCellString(indexFieldId, recordId);
  } else if (nameType === 2) {
    oCellIndexInfo = await oTable.getCellString(nameFieldId, recordId);
  }
